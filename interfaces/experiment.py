...
from utils.experiment_utils.experiment_constants import (ExperimentPhases,
                                                         ExperimentTypes,
                                                         Purposes)
...
class Experiment(ABC):
    ...
    def __init__(self, model: Network, args: argparse.Namespace, name: str) -> None:
        ...
        """
        experiment_mapping = {
            member.value.upper(): member for member in ExperimentTypes
        }
        ...
        self.experiment_type: ExperimentTypes = experiment_mapping[
            args.experiment_type.upper()
        ]
        ...
        if not os.path.exists(self.RESULT_PATH):
            ...
            print(
                f"Experiment '{self.EXP_NAME}/Output' result folder created successfully."
            )
            print(
                f"Experiment '{self.EXP_NAME}/Hidden' result folder created successfully."
            )
        else:
            try:
                ...
                os.makedirs(f"{self.RESULT_PATH}/Hidden", exist_ok=True)
                print(
                    f"Experiment {self.EXP_NAME} result folder re-created successfully."
                )
                print(
                    f"Experiment '{self.EXP_NAME}/Output' result folder re-created successfully."
                )
                print(
                    f"Experiment '{self.EXP_NAME}/Hidden' result folder re-created successfully."
                )
            except OSError as e:
        ...
        self.PRINT_LOG: logging.Logger = configure_logger(
            "Print Log", f"{self.RESULT_PATH}/prints.log"
        )  # Replace print statements (for debugging purposes)
        self.TEST_LOG: logging.Logger = configure_logger(
            "Test Log", f"{self.RESULT_PATH}/testing_accuracy.log"
        )  # Testing accuracy
        self.TRAIN_LOG: logging.Logger = configure_logger(
            "Train Log", f"{self.RESULT_PATH}/training_accuracy.log"
        )  # Training accuracy
        self.PARAM_LOG: logging.Logger = configure_logger(
            "Parameter Log", f"{self.RESULT_PATH}/parameters.log"
        )  # Experiment parameters
        self.DEBUG_LOG: logging.Logger = configure_logger(
            "Debug Log", f"{self.RESULT_PATH}/debug.log", level=logging.DEBUG
        )  # Debugging stuff
