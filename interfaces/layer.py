...
from utils.experiment_utils.experiment_constants import LayerNames, ParamInit
...
class NetworkLayer(nn.Module, ABC):
    ...
    def __init__(
        ...
        self.fc: nn.Linear = nn.Linear(
            self.input_dimension, self.output_dimension, bias=True
        )
    ...
    def _train_forward(
        self, input: torch.Tensor, clamped_output: Optional[torch.Tensor] = None
    ) -> torch.Tensor:
    ...
    def visualize_weights(
        self, result_path: str, num: int, use: str, fname: str
    ) -> None:
        """
        METHOD
        ...
        for ele in range(row * col):
            if ele < self.output_dimension:
                for ch in range(channels):
                    channel_weights = (
                        weight[ele, ch * input_dim : (ch + 1) * input_dim]
                        .cpu()
                        .detach()
                    )
                    ...
                    ax_col = (
                        ele % col
                    ) * channels + ch  # Ensure the column index is calculated correctly
    ...
    def get_norm(weights: torch.Tensor) -> torch.Tensor:
        norm: torch.Tensor = torch.norm(weights, p=2, dim=-1, keepdim=True).to(
            weights.device
        )
