...
class DataSetupLayer(InputLayer):
    ...
    def apply_color(image, color):
        ...
        brightness = (
            image.numpy().squeeze()
        )  # Get brightness from MNIST grayscale image
    ...
    def setup_colored_mnist(
        ...
        data_frame: pd.DataFrame = pd.read_csv(
            filename, header=None, on_bad_lines="skip"
        )
        labels: torch.Tensor = torch.tensor(data_frame[0].values)
        data_tensor: torch.Tensor = torch.tensor(
            data_frame.drop(data_frame.columns[0], axis=1).values, dtype=torch.float
        ).reshape(-1, 28, 28)
        data_tensor /= 255
        ...
        for i in range(data_tensor.shape[0]):
            ...
            for color in colors:
                ...
                colored_image = DataSetupLayer.apply_color(
                    data_tensor[i], {"rgb": rgb_values}
                )
                colored_data.append(
                    torch.tensor(colored_image, dtype=torch.float32).permute(2, 0, 1)
                )
                colored_labels.append(digit)  # Append the label for each colored copy

        colored_data_tensor = torch.stack(
            colored_data
        )  # Stack all colored images into a tensor
        colored_labels_tensor = torch.tensor(
            colored_labels, dtype=torch.long
    ...
    @staticmethod
    def setup_data(
        data: str, label: str, filename: str, size: int, dataset: DataSets
    ) -> TensorDataset:
        """
        STATIC METHOD
        ...
        labels: torch.Tensor = (
            torch.tensor(data_frame[0].values)
            if dataset != DataSets.E_MNIST
            else torch.tensor(data_frame[0].values) - 1
        )
        data_tensor: torch.Tensor = torch.tensor(
            data_frame.drop(data_frame.columns[0], axis=1).values, dtype=torch.float
        )
    ...
    def filter_data_loader(data_loader: DataLoader, filter: dict[int, int]):
        ...
        filtered_data_loader = DataLoader(
            filtered_dataset, batch_size=data_loader.batch_size, shuffle=True
        )
    ...
    def generate_bar_matrix(
        n: int, horizontal_indices: List[int], vertical_indices: List[int]
    ...
    def generate_training_set(
        n: int, k: int, forbidden_combinations: List[Tuple[int, int]]
    ) -> TensorDataset:
        """
        ...
        for _ in range(num_samples):
            while True:
                ...
                violation = any(
                    (h, v) in forbidden_combinations
                    for h in horizontal_indices
            ...
            matrix, col_label, row_label = DataSetupLayer.generate_bar_matrix(
                n, horizontal_indices, vertical_indices
            )
        ...
        matrices_tensor = torch.tensor(matrices_np).unsqueeze(
            1
        )  # Then convert to a PyTorch tensor

        col_labels_np = np.array(
            col_labels
        )  # Convert the list of numpy arrays to a single numpy array
        col_labels_tensor = torch.tensor(
            col_labels_np
        )  # Then convert to a PyTorch tensor

        row_labels_np = np.array(
            row_labels
        )  # Assuming you also have `row_labels` that need conversion
    ...
    @staticmethod
    def generate_test_set_one(n: int, k: int) -> TensorDataset:
        """
    ...
    @staticmethod
    def generate_test_set_three(n: int, k: int, max_bars: int) -> TensorDataset:
        """
